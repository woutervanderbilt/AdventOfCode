...
internal class Dag20 : Problem
{
    ...
    public override Task ExecuteAsync()
    {
        ...
        grid[0, 0] = true;
        Stack<HashSet<(int, int, int)>> stack = new Stack<HashSet<(int, int, int)>>();
        Stack<HashSet<(int, int, int)>> choicesStack = new Stack<HashSet<(int, int, int)>>();
        HashSet<(int x, int y, int doors)> currentPositions = new HashSet<(int, int, int)> { (0, 0, 0) };
        HashSet<(int, int, int)> choices = new HashSet<(int, int, int)>();
        IDictionary<(int, int), int> minDoors = new Dictionary<(int, int), int>();
        foreach (var c in input)
        {
            HashSet<(int, int, int)> newCurrentPositions = new HashSet<(int, int, int)>();
            bool copyNewCurrentPositions = true;
            bool breakOutOfLoop = false;
            foreach (var currentPosition in currentPositions)
            {
                var position = (currentPosition.x, currentPosition.y);
                switch (c)
                {
                    case '^':
                        ...
                        break;
                    case 'N':
                        MoveInDirection(GridDirection.North);
                        break;
                    case 'E':
                        MoveInDirection(GridDirection.East);
                        break;
                    case 'S':
                        MoveInDirection(GridDirection.South);
                        break;
                    case 'W':
                        MoveInDirection(GridDirection.West);
                        break;
                    case '(':
                    ...
                    case ')':
                        stack.Pop();
                        breakOutOfLoop = true;
                        choices.Add(currentPosition);
                        newCurrentPositions = choices;
                        Console.WriteLine($"End choices at {string.Join(',', choices)}");
                        choices = choicesStack.Pop();
                        break;
                    case '|':
                        currentPositions = new HashSet<(int, int, int)>(stack.Peek());
                        choices.Add(currentPosition);
                        Console.WriteLine($"Add new choice at {currentPosition}");
                        copyNewCurrentPositions = false;
                        breakOutOfLoop = true;
                        break;
                    case '$':
                ...
                void MoveInDirection(GridDirection direction)
                {
                    ...
                    Move();
                    grid[position.x, position.y] = true;
                    newCurrentPositions.Add((position.x, position.y, currentPosition.doors + 1));
                    if (!minDoors.ContainsKey(position))
                    {
                        minDoors[position] = currentPosition.doors + 1;
                    }
                    else
                    {
                        minDoors[position] = Math.Min(currentPosition.doors + 1, minDoors[position]);
                    }
                    ...
                    void Move()
                    {
                        ...
                        {
                            GridDirection.North => grid.North(position).Location,
                            GridDirection.East => grid.East(position).Location,
                            GridDirection.South => grid.South(position).Location,
                            GridDirection.West => grid.West(position).Location
                        };
                    }
                }
        ...
        var current = (0, 0);
        Counter<(int, int)> doorCounter = new Counter<(int, int)>();
        IList<(int x, int y)> lastStep = new List<(int x, int y)>();
        HashSet<(int, int)> visited = new HashSet<(int, int)>();
        visited.Add(current);
        lastStep.Add(current);
        while (lastStep.Any())
        {
            var copy = lastStep.ToList();
            lastStep = new List<(int, int)>();
            foreach (var location in copy)
            {
                var doorCount = doorCounter[location];
                if (((location.x | location.y) & 1) == 1)
                {
                    doorCount++;
                }
                else
        ...
        Result = $"{doorCounter.Values.Max().ToString()}  {doorCounter.Keys.Count(c => c.Item1 % 2 == 0 && c.Item2 % 2 == 0 && doorCounter[c] >= 1000)}";
        return Task.CompletedTask;
    }

    public override int Nummer => 201820;
}
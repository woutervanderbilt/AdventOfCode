...
internal class Dag03 : Problem
{
    ...
    public override Task ExecuteAsync()
    {
        ...
        var currentRoboHouse = (0, 0);
        bool santasMove = true;
        ...
        foreach (var direction in input)
        {
            if (direction == '>')
            {
                if (santasMove)
                {
                    currentHouse = (currentHouse.Item1 + 1, currentHouse.Item2);
                }
                else
                {
                    currentRoboHouse = (currentRoboHouse.Item1 + 1, currentRoboHouse.Item2);
                }
            }
            else if (direction == 'v')
            {
                if (santasMove)
                {
                    currentHouse = (currentHouse.Item1, currentHouse.Item2 - 1);
                }
                else
                {
                    currentRoboHouse = (currentRoboHouse.Item1, currentRoboHouse.Item2 - 1);
                }
            }
            else if (direction == '<')
            {
            ...
            houses.Add(currentHouse);
            houses.Add(currentRoboHouse);
            santasMove = !santasMove;
        }

        Result = houses.Count.ToString();
        return Task.CompletedTask;
    }

    public override int Nummer => 201503;
}